...
@Component({
  selector: 'app-patheykram-written-test',
  templateUrl: './patheykram-written-test.component.html',
  styleUrls: ['./patheykram-written-test.component.css']
})
export class PatheykramWrittenTestComponent implements OnInit {
      ...
      this.optionQA.map((data ,i)=>{
        ...
        if(i == this.optionQA.length - 1 ){
          ...
          this.coreService.post('written-test/add_questions', {'testquestion' : submitData}).subscribe((data: any) => {
            ...
            if (!data.error) {
              // console.log(data.id);
              this.router.navigate(['/patheykram/written-test-question']);
