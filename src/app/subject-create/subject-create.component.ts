...
preview() {
  ...
  create_patheykram(){
    ...
    if(this.createpatheykramForm.invalid){
      ...
      document.getElementById("preloader").style.display = "block";
      this.coreService.post('subject/create_subject' , createpatheykram).subscribe((data: any) => {
        document.getElementById("preloader").style.display = "none";
        console.log(data);
        if (!data.error) {
          this.router.navigate(['/patheykram' , this.id]);
          this.toastrService.success('SucessFull', 'subject Sucessfully created');
        } else {
          console.log(!data.message)
        }
      });
